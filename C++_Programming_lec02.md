...
p.41 Control Structures
------------------------
```
• 특별한 일이 발생하지 않으면 프로그램이 순차적으로 실행됩니다.
• 특별한 일이 일어나길 원할 때 우리는 통제 구조를 사용해야한다.
• 통제 구조는 두 가지 기본 기능을 제공합니다 : 선택 및 반복
```

p.42 Selection
---------------
```
• 선택 통제 구조는 대안의 행동 과정 중에서 선택하는 데 사용됩니다.
• 행동이 발생하는지 여부를 결정하는 조건이 있어야합니다.
• C ++에는 여러 선택 제어 구조가 있습니다.
- if
- if/else
- switch
```

p.43 Repetition Control Structures
-----------------------------------
```
• 반복 제어 구조는 일련의 동작 (명령문)을 반복 할 수있게 해줍니다.
• C ++은 여러 반복 제어 구조를 지원합니다.
  - while
  - for
  - do / while
```
